...
import com.example.lab_mobile.todo.data.remote.ItemEvent
        ...
        try {
            ...
            if (pendingOps.isNotEmpty()) {
                ...
                try {
                    ...
                    Log.d(TAG, "refresh: Received ${serverItems.size} items from server (with pending ops)")
            ...
            val serverItems = itemService.find(authorization = getBearerToken())
            itemDao.deleteAll()
            serverItems.forEach { itemDao.insert(it.copy(needsSync = false)) }

            Log.d(TAG, "refresh succeeded: Synced ${serverItems.size} items from server")
        } catch (e: Exception) {
